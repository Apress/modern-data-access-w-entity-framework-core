...
namespace EFC_Console
{
 class SQLSPTVF
 {
  ...
  /// <summary>
  /// new in EF Core 2.1 Preview 1!
  /// Not possible in EFC Core 1.0, 1.1 and 2.0: Non-entity classes as a result set
  /// </summary>
  [EFCBook]
  public static void Demo_SQLDirekt_NonEntityType()
  {
   ...
   using (var ctx = new WWWingsContext())
   {
    ...
    // this does not work:  var flightSet = ctx.Set<FlightDTO>().FromSql<fligthDTO>("Select FlightNo, Departure, Destination, Date from Flight");
    var flightSet1 = ctx.FlightDTO.FromSql("Select top 100 FlightNo, Departure, Destination, FlightDate as Date from Flight");
    var flightSet2 = ctx.Query<FlightDTO>().FromSql("Select top 100 FlightNo, Departure, Destination, FlightDate as Date from Flight");
    Console.WriteLine(flightSet1.Count());
    foreach (var f in flightSet1)
    {
     Console.WriteLine(f);
    }
    Console.WriteLine(flightSet1.Count());
