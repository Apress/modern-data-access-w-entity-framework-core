 ...
 public class EFC2_GlobalFilters
 {
  ...
  public static void GlobalFilter()
  {
   ...
   using (var ctx = new WWWingsContext(GlobalQueryFilter: true))
   {
    ...
    List<Flight> flightSet2 = ctx.FlightSet.FromSql("select top 100 * from Flight where Departure = 'Berlin'").ToList();
    ...
    CUI.Headline("TVF");
    List<Flight> flightSet3 = ctx.FlightSet.FromSql("Select top 100 * from GetFlightsFromTVF({0})", "Berlin").Where(f=>f.NonSmokingFlight == true).ToList();
    ...
    var pilotWithFlights = ctx.PilotSet.Include(x => x.FlightAsPilotSet).Take(10).ToList();
    ...
    var pilotenSet = ctx.PilotSet.Take(10).ToList();
