...
namespace EFC_Console
{
 class EFC_LINQ_CheatSheet
 {
  ...
  static public void Run()
  {
   ...
   // ========================= 
   CUI.Headline("Distinct");
   int dis1 = (from f in ctx.FlightSet select f.Departure).Distinct().Count();
   int dis1b = ctx.FlightSet.Select(x=>x.Departure).Distinct().Count();


   // WRONG:
   //int dis2 = (from f in ctx.FlightSet select f.Departure.Distinct()).Count();
   //int dis2b = ctx.FlightSet.Select(x => x.Departure.Distinct()).Count();
