...
namespace DA
{
 ...
 public class WWWingsContext : DbContext
 {
  public static string ConnectionString { get; set; } =
   @"Server=.;Database=WWWingsV2_EN;Trusted_Connection=True;MultipleActiveResultSets=True;App=Entityframework";
  ...
  protected override void OnModelCreating(ModelBuilder modelBuilder)
  {
   ...
   modelBuilder.Entity<Flight>().Property(x => x.Departure).HasDefaultValue("(unknown)");
   modelBuilder.Entity<Flight>().Property(x => x.Destination).HasDefaultValue("(unknown)");
   ...
   {
    CUI.PrintWarning("Global filter active!");
    ...
    //builder.Entity<Flight>().HasQueryFilter(x => x.FreeSeats > 0);
   }
   #endregion
   ...
   #endregion
   #region Columns ending in "No" become primary keys without auto increment
   foreach (IMutableEntityType entity in modelBuilder.Model.GetEntityTypes())
   {
   ...
   #region Relation Flight-- > FlugDetail
   ...
   //var ent2 = builder.Model.GetEntityTypes().SingleOrDefault(x => x.Surname == "GO.Flugdetail");
   ...
   //b.Property(x => x.FlightNo).HasColumnName("PK_FlugNr");
